...
public class FastqGenerator {
  public enum Scoring {
    ...
    private char getPhredQ(double p) {
      return (char) (-10 * Math.log10(p));
    }

    private char getLoggodsQ(double p) {
  ...
  protected void produce(int count) {
  ...
  protected String getReadname(int number) {
    return String.format("FASTQ_GENERATOR:%09d/1", number);
  }

  protected String getQuals(int number) {
    ...
    for (int i = 0; i < value.length; ++i) {
      ...
      int v = (int) (q + offset);
      if (v < '!' || v > '~') throw new RuntimeException("range exceeded: " + v);
      value[i] = (char) v;
    }
  ...
  protected String getRead(int number) {
    char value[] = new char[size];
    for (int i = 0; i < value.length; ++i)
  ...
  protected void emit(String readname, String reads, String quals) {
  ...
  static class Params {
    ...
    @Parameter(
        names = {"--quality-scoring", "-qs"},
        description = "Type of quality scoring system",
        required = true)
    String quality_scoring;

    @Parameter(
        names = {"--quality-offset", "-qo"},
        description = "Quality offset",
        required = true)
    int quality_offset;

    @Parameter(
        names = {"--low-boundary", "-lb"},
        description = "Low boundary of quality interval in output",
        required = true)
    int lboundary;

    @Parameter(
        names = {"--high-boundary", "-hb"},
        description = "High boundary of quality interval in output",
        required = true)
    int hboundary;

    @Parameter(
        names = {"--count", "-c"},
        description = "Number of records to be produced",
        required = true)
    int count;

    @Parameter(
        names = {"--sequence-size", "-ss"},
        description = "Size of sequences",
        required = true)
    int sequence_size;
  }
}
