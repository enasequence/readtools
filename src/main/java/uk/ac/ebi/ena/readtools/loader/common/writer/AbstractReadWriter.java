...
public abstract class AbstractReadWriter<T1 extends Spot, T2 extends Spot>
    implements ReadWriter<T1, T2> {
  ...
  public AbstractReadWriter<T1, T2> setVerbose(boolean verbose) {
  ...
  @Override
  public void setWriter(ReadWriter<T2, ? extends Spot> readWriter) {
    if (!readWriter.equals(this)) this.readWriter = readWriter;
  }

  public abstract String getKey(T1 spot) throws ReadWriterException;

  public abstract T2 assemble(final String key, List<T1> list) throws ReadWriterException;

  public void append(List<T1> list, T1 spot) throws ReadWriterException {
    list.add(spot);
  }
  ...
  public abstract boolean isCollected(List<T1> list);

  public void cascadeErrors() throws ReadWriterException {
    for (Entry<String, List<T1>> entry : spots.entrySet()) {
      if (null != readWriter) readWriter.write(handleErrors(entry.getKey(), entry.getValue()));
      else System.out.println("<?> " + handleErrors(entry.getKey(), entry.getValue()));
    }

    if (null != readWriter) readWriter.cascadeErrors();
  }

  public abstract T2 handleErrors(final String key, List<T1> list) throws ReadWriterException;

  public void write(T1 spot) throws ReadWriterException {
