...
class ReferenceSequenceFromSeekable {
  ...
  private ReferenceSequenceFromSeekable(
      SeekableStream s, Map<String, FastaSequenceIndexEntry> index) {
    this.s = s;
    this.index = index;
  }
  ...
  public byte[] getSubsequenceAt(String contig, long start, long stop) {
    ...
    while (targetBuffer.position() < length) {
      ...
      try {
        ...
        throw new SAMException(
            "Unable to load " + contig + "(" + start + ", " + stop + ") from " + s.getSource());
      }
  ...
  public static ReferenceSequenceFromSeekable fromString(String urlOrPathString) {
    try {
      ...
      SeekableStream indexSS =
          SeekableStreamFactory.getInstance().getStreamFor(urlOrPathString + ".fai");
