...
public enum QualInterval {
  ...
  protected final int lbound;
  protected final int hbound;

  protected final int typical_lbound;
  protected final int typical_hbound;
  protected final int offset;

  private QualInterval(int lbound, int hbound, int typical_lbound, int typical_hbound, int offset) {
    this.lbound = lbound;
    this.hbound = hbound;
  ...
  double getWeight(long[] interval, int offset) {
    long tcount = 0;
    long icount = 0;
    for (int i = 0; i < interval.length; ++i) {
      ...
      tcount += count;
      int pos = i + offset;
      if (count > 0 && pos >= typical_lbound && pos <= typical_hbound) {
        icount += count;
      }
    }
  ...
  public static Map<QualInterval, Double> forRow(long[] interval, int offset) {
    int from_interval = 0;
    ...
    for (QualInterval i : QualInterval.values()) {
      if (from_interval >= i.lbound && to_interval <= i.hbound)
