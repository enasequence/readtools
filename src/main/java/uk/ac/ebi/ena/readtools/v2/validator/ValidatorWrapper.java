...
public class ValidatorWrapper {
  ...
  private void validateMultipleFastq(List<File> files) throws ReadsValidationException {
    try {
      /**
       * Should ideally have high number of duplicates as it will point to higher pairing
       * percentage. To keep memory consumption lower, because we can tolerate false positive here,
       * use lower expected read size.
       */
      BloomWrapper mainFileOnlyPairingBloomWrapper = new BloomWrapper(readCountLimit / 2);
      validateInsdc(files.get(0));

      PairedFastqReadsValidator mainValidator =
          new PairedFastqReadsValidator(
              readCountLimit,
              files.get(0).getAbsolutePath(),
              mainFileOnlyPairingBloomWrapper,
              labels);
      ...
      for (int fileNumber = 1; fileNumber < files.size(); fileNumber++) {
        ...
        // Make a copy of the main file's pairing information so we do not have re-create it for
        // every other file.
        BloomWrapper bloomWrapper = mainFileOnlyPairingBloomWrapper.getCopy();

        PairedFastqReadsValidator secondaryValidator =
            new PairedFastqReadsValidator(
      ...
      if (labels.size() <= files.size()) {
        ...
        if (lowestPairingPercentagePair.pairingPercentage < (double) PAIRING_THRESHOLD) {
          ...
          //                            String.format("Detected paired fastq submission with less
          // than %d%% of paired reads between %s and %s",
          //                                    PAIRING_THRESHOLD,
          // lowestPairingPercentagePair.fileName1, lowestPairingPercentagePair.fileName2)));
        ...
        //                        "When submitting paired reads using two Fastq files the reads must
        // follow Illumina paired read naming conventions. "
        //                                + "This was not the case for the submitted Fastq files:
        // %s. Unable to determine pairing from set: %s",
        //                        files,
        //                        labels.stream().limit(10).collect(Collectors.joining(",", "", 10 <
        // labels.size() ? "..." : "")))));
        throw new ReadsValidationException(
            String.format(
                ...
                labels.stream()
                    .limit(10)
                    .collect(Collectors.joining(",", "", 10 < labels.size() ? "..." : ""))));
      }
    } catch (ReadsValidationException rve) {
      throw rve;
    } catch (Exception e) {
  ...
  private void validateInsdc(File file) throws ReadsValidationException {
    ...
    fileQualityStats.add(
        new FileQualityStats(
            file,
            insdcReadsValidator.getReadCount(),
            insdcReadsValidator.getHighQualityReadCount()));
  }
