...
public class Sam2Fastq {
  ...
  public void create(Params params) throws Exception {
    ...
    if (params.reference == null)
      log.warn(
          "No reference file specified, remote access over internet may be used to download public sequences. ");

    final AtomicBoolean brokenPipe = new AtomicBoolean(false);
    try {
      sun.misc.Signal.handle(
          new sun.misc.Signal("PIPE"),
  ...
  private abstract static class Dumper implements Runnable {
    ...
    public Dumper(
      ...
      for (int index = 0; index < outputs.length; index++) outputs[index] = new FileOutput();

      if (fastqBaseName == null) {
        OutputStream joinedOS = System.out;
        if (gzip) joinedOS = (new GZIPOutputStream(joinedOS));
        for (int index = 0; index < outputs.length; index++) outputs[index].outputStream = joinedOS;
      } else {
        String extension = ".fastq" + (gzip ? ".gz" : "");
        String path;
        for (int index = 0; index < outputs.length; index++) {
          if (index == 0) path = fastqBaseName + extension;
          else path = fastqBaseName + "_" + index + extension;
    ...
    protected void doRun() throws IOException {
      ...
      final SamReader samReader =
          SamReaderFactory.makeDefault().referenceSource(referenceSource).open(samFile);
      ...
      for (final SAMRecord currentRecord : samReader) {
        ...
        byte[] readBases =
            Arrays.copyOf(currentRecord.getReadBases(), currentRecord.getReadBases().length);
        byte[] baseQualities =
            currentRecord.getBaseQualityString().getBytes(StandardCharsets.UTF_8);
        ...
        fastqOutputter.writeRead(
            readName.getBytes(StandardCharsets.UTF_8),
            currentRecord.getFlags(),
            readBases,
            baseQualities);
      }
      ...
      if (!brokenPipe.get()) fastqOutputter.finish();
    }
    ...
    public void run() {
      try {
        ...
        if (outputs != null) {
          for (FileOutput os : outputs) os.close();
        }
      } catch (Exception e) {
        this.exception = e;
      }
  ...
  private static class CollatingDumper extends Dumper {
    ...
    public CollatingDumper(
      ...
      fo.file =
          File.createTempFile(
              fastqBaseName == null ? "overflow.bam" : fastqBaseName + ".overflow.bam", ".tmp");
  ...
  private static class FileOutput extends OutputStream {
    ...
    public void close() throws IOException {
      ...
      if (empty && file != null && file.exists()) file.delete();
    }
  }
  ...
  public static class Params {
    @Parameter(
        ...
        converter = CramTools.LevelConverter.class)
    ...
    @Parameter(
        names = {"--input-sam-file", "-I"},
        converter = FileConverter.class,
        description = "The path to the SAM file to uncompress. Omit if standard input (pipe).")
    public File samFile;

    //		@Parameter(names = { "--reference-fasta-file", "-R" }, converter = FileConverter.class,
    // description = "Path to the reference fasta file, it must be uncompressed and indexed (use
    // 'samtools faidx' for example). ")
    File reference;

    @Parameter(
        names = {"--fastq-base-name", "-F"},
        description =
            "'_number.fastq[.gz] will be appended to this string to obtain output fastq file name. If this parameter is omitted then all reads are printed with no garanteed order.")
    ...
    @Parameter(
        names = {"--reverse"},
        description = "Re-reverse reads mapped to negative strand.")
    public boolean reverse = false;

    @Parameter(
        names = {"--enumerate"},
        description =
            "Append read names with read index (/1 for first in pair, /2 for second in pair).")
    public boolean appendSegmentIndexToReadNames;

    @Parameter(
        names = {"--max-records"},
        description = "Stop after reading this many records.")
    public long maxRecords = -1;

    @Parameter(
        names = {"--read-name-prefix"},
        description = "Replace read names with this prefix and a sequential integer.")
    public String prefix = null;

    @Parameter(
        names = {"--default-quality-score"},
        description =
            "Use this quality score (decimal representation of ASCII symbol) as a default value when the original quality score was lost due to compression. Minimum is 33.")
    public int defaultQS = '?';

    @Parameter(
        names = {"--ignore-md5-mismatch"},
        description =
            "Issue a warning on sequence MD5 mismatch and continue. This does not garantee the data will be read succesfully. ")
    public boolean ignoreMD5Mismatch = false;

    @Parameter(
        names = {"--skip-md5-check"},
        description = "Skip MD5 checks when reading the header.")
