...
public class Utils {
  ...
  public static boolean quitOnMissingEOF =
      Boolean.parseBoolean(System.getProperty("debug.quit-on-missing-eof", "true"));

  public static void checkRefMD5(
      SAMSequenceDictionary d,
      ReferenceSequenceFile refFile,
      boolean checkExistingMD5,
      boolean failIfMD5Mismatch)
      ...
      if (!r.getAttributes().contains(SAMSequenceRecord.MD5_TAG)) {
        ...
        if (checkExistingMD5) {
          ...
          if (!md5.equals(existingMD5)) {
            ...
            String message =
                String.format(
                    "For sequence %s the md5 %s does not match the actual md5 %s.",
                    r.getSequenceName(), existingMD5, md5);

            if (failIfMD5Mismatch) throw new RuntimeException(message);
