package uk.ac.ebi.ena.sampler.intervals;
...
QualScoring
{
    ...
    final static int    hbound = 41;
    final static int    lbound = -5;
    ...
    QualScoring( int type )
    {
      ...
      //      System.out.printf( "%d:probs[ %d ] = %f\n", type, i + lbound, probs[ i ] );
        ...
        for( int i = 0; i < ps.length; ++i )
            disp += Math.pow( ( i + lbound ) - math_e, 2 ) * probs[ i ];
        disp = Math.sqrt( disp );
        System.out.printf( "sum: %f, exp: %f, disp: %f, avg: %f\n", sum, math_e, disp, sum / (double)ps.length );
    }
    
    
    public double
    calcWeight( long[] row, int offset )
    {
        double result = 0;
        long tcount = 0;
...
//                || ( i + lbound ) == 2 )// 2 - Illumina 1.5 /B/
            ...
            System.out.printf( "%7s: probs[%3d] = %f\t%f\t%f\n", 
                               this.toString(), 
                               i + lbound,  
                               probs[ i ], 
                               p, 
    ...
    public static Map<QualScoring, Double>
    forRow( long[] row, int offset )
    { 
        Map<QualScoring, Double> map = new HashMap<QualScoring, Double>();
        for( QualScoring qs : QualScoring.values() )
            map.put(  qs, qs.calcWeight( row, offset ) );
