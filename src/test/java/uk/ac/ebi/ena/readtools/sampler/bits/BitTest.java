...
public class BitTest {
  // @Test
  public void BitStreamTest() throws IOException {
  ...
  @Test
  public void DictStreamTest() throws IOException {
    ByteArrayOutputStream bytes = new ByteArrayOutputStream();
    DictionaryOutputStream bstream =
        new DictionaryOutputStream(new BitOutputStream(bytes), new BaseDict());
    String seq = "ACGTNTGCAN";
    bstream.write(seq.getBytes());

    bstream.close();

    DictionaryInputStream bis =
        new DictionaryInputStream(
            new BitInputStream(new ByteArrayInputStream(bytes.toByteArray())), new BaseDict());
    System.out.println("Stream length: " + bytes.toByteArray().length);
    ...
    try {
      while (-1 != (ch = bis.read())) sb.append((char) ch);
